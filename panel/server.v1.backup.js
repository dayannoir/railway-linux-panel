...
$('loadFiles').onclick=loadFiles;$('newFolder').onclick=async()=>{const name=prompt('نام فولدر جدید:');if(!name)return;try{const base=$('filePath').value||'.';const target=(base==='.'?'':base+'/')+name;await api('/api/files/mkdir',{method:'POST',body:JSON.stringify({path:target})});loadFiles()}catch(e){alert(e.message)}};$('newFile').onclick=async()=>{const name=prompt('نام فایل جدید:');if(!name)return;try{const base=$('filePath').value||'.';const target=(base==='.'?'':base+'/')+name;await api('/api/files/create',{method:'POST',body:JSON.stringify({path:target})});loadFiles()}catch(e){alert(e.message)}};$('uploadBtn').onclick=()=>$('uploadFile').click();$('uploadFile').onchange=async()=>{const f=$('uploadFile').files[0];if(!f)return;const base=($('filePath').value||'.').replace(/^\\.|\\/$/g,'');const target=(base?base+'/':'')+f.name;const r=await fetch('/api/files/upload?path='+encodeURIComponent(target),{method:'POST',headers:{'x-csrf-token':csrf,'content-type':'application/octet-stream'},body:await f.arrayBuffer()});if(!r.ok){const j=await r.json();alert(j.error||'آپلود ناموفق بود')}else loadFiles()};$('saveFile').onclick=async()=>{try{await api('/api/files/save',{method:'POST',body:JSON.stringify({path:$('editPath').value,content:$('editor').value})});alert('ذخیره شد')}catch(e){alert(e.message)}};
...
async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/api/files/mkdir') { if (!requireChanged(s,res)) return; const input=await jsonBody(req); const dir=safePath(input.path); if(dir===DATA_ROOT||dir===PANEL_ROOT) throw new Error('Invalid folder path'); await fsp.mkdir(dir); return json(res,200,{ok:true,path:relative(dir)}); }
    if (req.method === 'POST' && url.pathname === '/api/files/create') { if (!requireChanged(s,res)) return; const input=await jsonBody(req); const file=safePath(input.path); if(file===DATA_ROOT||file===PANEL_ROOT) throw new Error('Invalid file path'); await fsp.mkdir(path.dirname(file),{recursive:true}); const handle=await fsp.open(file,'wx'); await handle.close(); return json(res,200,{ok:true,path:relative(file)}); }
